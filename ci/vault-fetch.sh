#!/usr/bin/env bash
# Fetch CI secrets from a vault via a least-privilege AppRole.
#
# Spec format, one directive per line:
#   env     <ENV_NAME> <mount/data/path or ${PATH_VAR}> <field or ${FIELD_VAR}>
#   keyfile <ENV_NAME> <mount/data/path or ${PATH_VAR}> <field or ${FIELD_VAR}>
#
...
Spec directives:
  env <ENV_NAME> <mount/data/path> <field>
  keyfile <ENV_NAME> <mount/data/path> <field>

Example:
  env GHCR_TOKEN ${GHCR_TOKEN_VAULT_PATH} ${GHCR_TOKEN_VAULT_FIELD}
  keyfile DEPLOY_KEY_FILE ${DEPLOY_KEY_VAULT_PATH} ${DEPLOY_KEY_VAULT_FIELD}
EOF
}
...
token="$(
  ...
  curl -sk --max-time 15 -X POST --data @- "$VAULT_ADDR/v1/auth/approle/login" |
  jq -r '.auth.client_token // empty'
)"
[[ -n "$token" ]] || die "AppRole login failed"
mask "$token"

revoke_token() {
  [[ -n "${token:-}" ]] || return 0
  curl -sk --max-time 10 -H "X-Vault-Token: $token" \
...
while read -r kind name path field extra; do
  ...
  [[ "$path" == */* ]] || die "path must be mount/data/path: $path"

  value="$(
    curl -sk --max-time 15 -H "X-Vault-Token: $token" "$VAULT_ADDR/v1/$path" |
