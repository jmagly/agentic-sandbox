...
grep -Fq 'needs: [release-binaries, release-linux-packages, release-macos-promote, docker, integration]' \
  "$ci"
grep -Fq 'scripts/verify-macos-developer-bundle.sh' "$ci"
grep -Fq 'DEVELOPER_RELEASE_TAG: ${{ vars.MACOS_DEVELOPER_RELEASE_TAG }}' "$ci"
grep -Fq 'DEVELOPER_PACKAGE_SHA256: ${{ vars.MACOS_DEVELOPER_PACKAGE_SHA256 }}' "$ci"
grep -Fq 'DEVELOPER_MANIFEST_SHA256: ${{ vars.MACOS_DEVELOPER_MANIFEST_SHA256 }}' \
  "$ci"
grep -Fq '[[ "$DEVELOPER_RELEASE_TAG" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]' "$ci"
grep -Fq '[[ "$GITHUB_SHA" =~ ^[0-9a-f]{40}$ ]]' "$ci"
grep -Fq -- '--bundle-dir "developer-staging/${DEVELOPER_PACKAGE_SHA256}"' "$ci"
grep -Fq -- '--expected-package-sha256 "$DEVELOPER_PACKAGE_SHA256"' "$ci"
grep -Fq -- '--expected-manifest-sha256 "$DEVELOPER_MANIFEST_SHA256"' "$ci"
grep -Fq -- '-name '\''*.evidence.json'\''' "$ci"
grep -Fq -- '-name '\''SHA256SUMS-macos-developer'\''' "$ci"
