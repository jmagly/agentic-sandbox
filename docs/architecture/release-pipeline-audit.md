...
- `release-binaries-mutsu` job (tag-only) builds Apple Silicon host-direct artifacts on mutsu over SSH and fails closed when `MUTSU_SSH_KEY` is absent. The current macOS package is the `aarch64-darwin` tarball containing `sandboxctl`, `agent-client`, docs, and an `MGMT_EXCLUDED.txt` note. A `.dmg` is intentionally deferred: this project has no macOS desktop app bundle yet, and `agentic-mgmt`/Apple VM provider support remains tied to #438.
- `release-attach` job (tag-only, gates on `release-binaries` + `release-binaries-mutsu` + `release-linux-packages` + `docker` + `integration`) downloads the matrix artifacts, generates a canonical `SHA256SUMS` file across tarballs, native packages, and installer script, creates the Gitea release, and attaches every release asset plus checksum sidecars.
...
- [#299](https://github.com/jmagly/agentic-sandbox/issues/299) / [#478](https://git.integrolabs.net/roctinam/agentic-sandbox/issues/478) — `multi-registry-push` job. Mirrors the release-tagged runtime image set (`agentic-mgmt`, `agent-client`, `agent`, `claude`, `codex`, `opencode`, `automation-control`) to public package names under `ghcr.io/<owner>/agentic-sandbox-*` and optional Quay equivalents. GHCR is release-blocking; optional mirrors skip only when explicitly documented.
...
- [ ] Apple Silicon host-direct tarball is built on mutsu, smoke-tested on mutsu, and release-blocking
