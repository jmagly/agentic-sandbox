...
- `release-binaries-mutsu` is disabled while Darwin/macOS release artifacts are deferred. The mutsu SSH implementation remains in the workflow as implementation history, but it is not release-blocking and its SSH credential is not required for current production tags. A credential-free implementation slice now defines a production client-tools package as a Developer-ID-signed `.pkg` inside a signed/notarized/stapled `.dmg`; real Apple-builder signing proof and promotion remain gated. `agentic-mgmt`/Apple provider support remains tied to #438/#488/#489.
...
**Phase 2 status:**
- `aarch64-apple-darwin` — **implementation preview**; the mutsu SSH lane is disabled and is not part of the public release matrix. `scripts/package-macos.sh` now codifies the fail-closed signature, notarization, stapling, Gatekeeper, and checksum contract for a `sandboxctl`/`agent-client` `.pkg` + `.dmg`. Re-enable only after an eligible Apple builder and real signing proof are approved. Management/runtime support stays gated on #438/#488/#489.
