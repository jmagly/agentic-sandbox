...
Provider exit is offline-only. `scripts/celld-offline-migration.mjs` implements the provider-neutral state machine: enumerate and stop every writer class, deny application writes, require two identical listings, copy every key/body/metadata value, compare independent key-set, size, content, metadata, and combined hashes, canary the destination while source writes remain denied, rehearse direct rollback only before destination writes, record cutover, prove a post-cutover application write changed the durable manifest, and require the same quiesced verified process in reverse. Every mutation is planned and completed in a protected hash-chained journal. The controller rejects concurrent authorities only after observing both stores' write policy and fleet/writer state, and accepts only the `celld_object_store_only` scope; it cannot target local filesystems, volume mounts, workspaces, VM disks, agentshare, or management state.

`scripts/celld-live-offline-migration.mjs` is the Titan adapter for that state machine. It first subjects a distinct destination to the full 10,000-create/10,000-overwrite two-gateway S3-v1 qualification gate, deploys the reviewed Worker to both disposable stores, then seeds and changes Celld state through signed Worker commands rather than direct test writes. It toggles only the disposable run-bucket application policy, stops all three Celld nodes (including their alarm writers), records the deployment CLI and deliberately absent management-reconciler writer classes, and compares the complete run namespace in both directions. The success record binds the exact commit, host, Celld/Worker pins, backend pins/topologies, hashed bucket namespaces, cutover, mutation journal, two-sided authority observations, destination qualification artifacts, and cleanup result. The workflow verifies a SHA-256 manifest and uploads both aggregate and raw destination evidence.

This live rehearsal owns two newly generated, run-scoped fixtures under `/dev/shm`; it is not an operator migration command and never accepts an existing Sandbox storage path. Successful runs remove only exact label/marker-owned disposable fleets, Compose resources, and destination roots before the postflight baseline comparison. A failure after the migration journal starts instead removes the Celld fleets, makes both bucket identities read-only, stops both Compose projects without deleting their volumes, writes `artifacts/celld-offline-migration-error.json`, and deliberately fails the postflight residue gate. After reviewing that artifact and journal, an operator can reap the two exact fixture configs with the normal fleet and SeaweedFS cleanup commands. Deleting retained non-disposable source data remains a separately approved operator action.
