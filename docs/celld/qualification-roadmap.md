...
| #765 | Construction started; not live-ready. Versioned raw evidence and exact-run network inventories now bind a 32-probe ceiling, protected management-ledger counter deltas, container namespace inodes, directional targets, and unique nftables identities. A private mTLS proxy executable now requires the run CA plus the exact management leaf and forwards plaintext only to node loopback. Per-node certificate deployment, the negative certificate/bypass matrix, live partition matrices, and independently verified proxy cleanup are still absent. | Deploy the private mTLS proxy from the exact-run inventory, execute the negative certificate matrix, then apply/heal only the typed run-owned partition plans. |
