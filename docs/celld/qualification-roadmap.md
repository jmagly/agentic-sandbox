...
| #765 | Construction started; not live-ready. Versioned raw evidence and exact-run inventories now bind a 32-probe ceiling, protected management-ledger counter deltas, container namespace inodes, directional targets, unique nftables identities, and per-node mTLS proxy sidecars. The driver creates IP-SAN server identities plus one exact management leaf, persists every proxy before creation, forwards plaintext only to node loopback, and removes only exact owned sidecars. Management is not yet routed through those proxies; the negative certificate/bypass matrix and live partition matrices are still absent. | Route the management client through the private proxies, execute the negative certificate matrix, then apply/heal only the typed run-owned partition plans. |
