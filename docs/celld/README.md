...
The protected Titan qualification driver alone sets `AGENTIC_CELLD_QUALIFICATION_DISPATCH_GATE_DIR`. Do not set it for a normal deployment. When configured, management accepts gate requests only from an existing absolute, non-symlink, owner-only directory; each owner-only request binds the SHA-256 of one operation ID and either the `during_dispatch` or `after_dispatch` phase. Management publishes the exact phase, PID, and timestamp and waits at most 30 seconds for operator release. Missing requests have no effect, and malformed, weak-permission, mismatched, or timed-out gates fail closed. This deterministic fault seam lets UAT-004 kill the observed management process at a product-defined dispatch boundary without exposing an HTTP fault endpoint.

## API and CLI

The authenticated management surface is under `/api/v2/celld`: status, cells, commands, reconciliation, bundle validation, fleet validation, object-store evidence preflight, read-only fleet diagnosis, and advisory upgrade planning. `sandboxctl celld diagnose --file OBSERVATIONS.json` classifies a strict observation envelope; fixture/local inputs cannot claim live qualification. Upgrade plans are explicitly non-mutating until the CLQ-10 controller exists. InstanceCell alarms call the internal `POST /api/v2/celld/effects` boundary with a fresh, generation-bound HMAC and the original idempotency key. Management durably claims that identity before provider dispatch; terminal replays return the original result, while unknown outcomes are looked up and never assigned a replacement effect identity. An interrupted no-effect `observe` resumes from that durable dispatch claim without external provider work. Callback responses include `provider_dispatch_count`, the durable number of times management granted the operation's provider-dispatch owner. It is expected to remain one across retries and restarts; it records a dispatch boundary crossing, not an unsupported claim that the external provider completed the effect. Generation-fenced callbacks rejected before ledger claim also return `provider_dispatch_count_delta: 0`; this is a product-generated statement that the rejected callback did not cross the management-to-provider dispatch boundary, not a lifetime provider counter. `sandboxctl celld --help` exposes the operator diagnostics and validations.
