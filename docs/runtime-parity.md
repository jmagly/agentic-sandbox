...
| Fork from warm base | N/A | N/A | RAM copied per child | Supported | `ch-faststart.sh fork` launches children concurrently, rolls back partial fan-out, samples RAM after coexistence, and assigns per-child writable disks, UUIDs, bootstrap credentials, and VSock CIDs |
