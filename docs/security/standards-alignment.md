...
| NIST SP 800-53 Rev. 5 | SC, SR: system communications and supply chain risk | Local loopback defaults, transport identity, release checksums, optional signing/SBOMs, base-image verification tooling, and VM base/seed/loadout metadata hashes. Evidence: [security status](security-status.md), [release verification](../releases/verification.md), `images/qemu/iso-pins.json`. | Partial | SLSA/in-toto provenance is not claimed; complete release-profile verification remains required per release. |
| CISA Secure by Design | Secure defaults and customer burden reduction | Local-first management plane, loopback defaults, secure agent transport options, retired legacy shared-secret docs, checksum-verifying installer, Docker dev bind guidance, and explicit Docker readiness fields. Evidence: [attack surface inventory](attack-surface.md), [release verification](../releases/verification.md), `docs/API.md`, [getting started](../getting-started.md). | Partial | Docker dev mode still requires an intentional plaintext acknowledgement when containers must reach host HTTP bootstrap; remote admin hardening remains unclaimed. |
...
| OWASP ASVS | Authentication, session management, access control, API security | Local operator API docs, selected bearer-auth dispatch, transport identity docs, retired secret endpoints, dashboard CSP, and representative DOM-sink regression. Evidence: `docs/API.md`, [attack surface inventory](attack-surface.md), [ASVS profile](asvs-profile.md). | Partial | Remote multi-user HTTP/WS admin authentication and full dashboard hardening are not claimed. |
