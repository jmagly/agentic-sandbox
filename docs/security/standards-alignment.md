...
| OWASP ASVS | Authentication, session management, access control, API security | Local operator API docs, selected bearer-auth dispatch, transport identity docs, retired secret endpoints. Evidence: `docs/API.md`, [attack surface inventory](attack-surface.md), [ASVS profile](asvs-profile.md). | Partial | ASVS targets and control statuses are now published; HTTP/WS enforcement and dashboard CSP gaps remain tracked by #551/#552. |
