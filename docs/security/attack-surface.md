...
| Credential proxy | Workload to upstream web/API/Git/S3/registry/database service through host broker | Planned | Useful when protocol mediation avoids placing raw secrets in the guest. Not suitable for every provider or CLI. |
...
| Credential proxy backend | Workload calls upstream through broker | Planned | Preferred for web applications and systems where HTTP/Git/S3/registry/database flows can be mediated. |
