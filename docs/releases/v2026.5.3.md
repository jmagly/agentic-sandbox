...
- **[#312](https://git.integrolabs.net/roctinam/agentic-sandbox/issues/312)** — `build-base-image.sh` hits a `virt-install --cdrom + --extra-args` API incompatibility. titan can't currently produce the Ubuntu 24.04 base image the e2e harness needs. **E2E is gated on tag pushes only** until #312 ships and the qcow2 is staged at `/mnt/ops/base-images/ubuntu-server-24.04-agent.qcow2`. Branch pushes skip e2e entirely.
