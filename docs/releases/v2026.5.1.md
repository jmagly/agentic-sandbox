...
- **Binary release artifacts**: this release ships from source only. The following follow-on work is tracked and will land in a subsequent release:
  ...
  - [#298](https://git.integrolabs.net/roctinam/agentic-sandbox/issues/298) — PyPI publish for the Python SDK (P1)
...
- **Python SDK** (`sdk/python/`) stays at `0.1.0` — version alignment to CalVer happens when the PyPI publish workflow lands.
