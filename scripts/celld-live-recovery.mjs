...
export const RECOVERY_PREREQUISITES = Object.freeze([
  ...
  Object.freeze({ id: "CELLD_OBSERVABILITY_QUALIFICATION", status: "unavailable", reason_code: "CELLD_OBSERVABILITY_QUALIFICATION_UNAVAILABLE" }),
  Object.freeze({ id: "CELLD_VERSIONED_SNAPSHOT_FIXTURE", status: "unavailable", reason_code: "CELLD_VERSIONED_SNAPSHOT_RESTORE_UNAVAILABLE" }),
  Object.freeze({ id: "CELLD_INDEPENDENT_EVIDENCE_STORE", status: "unavailable", reason_code: "CELLD_INDEPENDENT_EVIDENCE_STORE_UNAVAILABLE" }),
]);

function unavailable(profile, runId, startedAt, prerequisites) {
  return {
    schema_version: OBSERVATION_SCHEMA, driver_id: DRIVER_ID, run_id: runId, scenario_id: SCENARIO_ID,
...
export function executeRecoveryDriver({ scenarioId, runId, liveProfilePath }, dependencies = {}) {
  ...
  return unavailable(profile, runId, startedAt, dependencies.prerequisites?.() ?? RECOVERY_PREREQUISITES);
}

async function main(args) {
  const observation = executeRecoveryDriver({ scenarioId: argument(args, "--scenario-id"), runId: argument(args, "--run-id"), liveProfilePath: resolve(argument(args, "--profile")) });
