...
export function validateNetworkAuthInventory(inventory, { runId, runRoot, hostSha256 } = {}) {
  ...
  for (const [index, proxy] of inventory.proxies.entries()) {
    ...
    const tlsRoot = join(inventory.run_root, "network-tls");
    if (!namespace || proxy?.name !== `${proxy.node_container}-mtls-proxy` || !CONTAINER_NAME.test(proxy.name) || proxyNames.has(proxy.name) || proxyNodes.has(proxy.node_container) || !validIpAddress(proxy?.listen_address) || proxy?.listen_port !== MTLS_PROXY_PORT || proxy?.target_address !== "127.0.0.1" || proxy?.target_port !== 8080 || !SHA256.test(proxy?.binary_sha256 ?? "") || !/^sha256:[0-9a-f]{64}$/.test(proxy?.image_ref ?? "") || proxy?.ca_file_ref !== join(inventory.run_root, "tls/ca.crt") || proxy?.server_cert_file_ref !== join(tlsRoot, `${proxy.node_container}-server.crt`) || proxy?.server_key_file_ref !== join(tlsRoot, `${proxy.node_container}-server.key`) || proxy?.management_client_cert_file_ref !== join(tlsRoot, "management-client.crt") || proxy?.management_client_identity_file_ref !== join(tlsRoot, "management-client.pem") || !["planned", "created", "started", "removed"].includes(proxy?.status) || !validTimestamp(proxy?.planned_at) || !validTimestamp(proxy?.updated_at) || (proxy?.status === "created" && !validTimestamp(proxy?.created_at)) || (proxy?.status === "started" && (!validTimestamp(proxy?.created_at) || !validTimestamp(proxy?.started_at))) || (proxy?.created_at !== undefined && !validTimestamp(proxy.created_at)) || (proxy?.started_at !== undefined && !validTimestamp(proxy.started_at)) || (proxy?.status === "removed" && !validTimestamp(proxy?.removed_at))) errors.push(`network inventory proxy is invalid at index ${index}`);
