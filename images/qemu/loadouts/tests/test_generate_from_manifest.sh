...
assert_contains "browser-qa scopes headless-shell userns" "/opt/carbonyl/headless_shell flags=(unconfined)" "$USERDATA"
assert_contains "browser-qa scopes operator-shell userns" "/opt/carbonyl/carbonyl_operator_shell flags=(unconfined)" "$USERDATA"
...
assert_contains "codex provider installs pinned npm package" "@openai/codex@0.149.0" "$USERDATA"
assert_contains "codex provider wires static PATH symlink" 'ln -sf "$CODEX_BIN" "$HOME/.local/bin/codex"' "$USERDATA"
assert_contains "copilot provider installs pinned npm package" "@github/copilot@1.0.80" "$USERDATA"
...
assert_contains "openclaw provider installs pinned npm package" "openclaw@2026.7.1-2" "$USERDATA"
